                            ...
                            foreach ($paginationData['records'] as $record) :
                                ...
                                foreach ($table_heading as $key => $value):
                                    ...
                                    <td class=" ">
                                        ...
                                        <?php
                                        if($key == 'category_self_id'){
                                            echo Functions::getCategoryTitle($record->$key);
                                        } else if ($key == 'status') {
                                            ?>
                                            <a href="<?php echo LINK_URL . CONTROLLER . "/status/" . $record->id; ?>"
                                               title="<?php echo $record->title; ?>"
                                               style="<?php echo ($record->$key) == '1' ? 'color:green' : 'color:red'; ?>"><?php echo ($record->$key) == '1' ? 'Active' : 'In-Active'; ?></a>
                                        <?php
                                        }else if ($key == 'approved') {
                                            ?>
                                            <a href="<?php echo LINK_URL . CONTROLLER . "/approve/" . $record->id; ?>"
                                               title="<?php echo $record->title; ?>"
                                               style="<?php echo ($record->$key) == '1' ? 'color:green' : 'color:red'; ?>"><?php echo ($record->$key) == '1' ? 'Approved' : 'Un-Approved'; ?></a>
                                        <?php
                                        }else{
                                            echo $record->$key;
                                        }
