...
class TopicsController extends MainController
{
    ...
    public function __construct()
    {
        ...
        $this->data['topics_model'] = $this->getModel("TopicsModel");
    ...
    /*public function getCategoryRelatedTutorial(){
        $tutorial_model = new TutorialModel();
        if(isset($_POST['id'])){
            $id = $_POST['id'];
            $tutorial_model->condition = "category_id=:id";
            $tutorial_model->params = array(":id"=>$id);
            $tutorial_result = $tutorial_model->findAll();
            //print_r($tutorial_result);


            $myJSON = json_encode($tutorial_result);
